...
paymentRouter.post("/stripe/create-intent", async (req, res) => {
  if (!env.stripeSecretKey) return res.status(503).json({ message: "Stripe key missing" });
...
paymentRouter.post("/razorpay/order", async (req, res) => {
  if (!env.razorpayKeyId || !env.razorpayKeySecret) return res.status(503).json({ message: "Razorpay keys missing" });
